...
class CoinsControllerFilterTest extends TestCase
{
    ...
    public function testSuccess(array $data): void
    {
        $response = $this->actingAs($this->getAdmim())
            ...
            ->get(route("admin.index") . $this->buildQuery($data));
    ...
    public function testEmptySuccess(): void
    {
        $response = $this->actingAs($this->getAdmim())
            ->withSession(['banned' => false])
            ->get(route("admin.index") . "?countries[]=&nominals[]=&coinages[]=&materials[]=&currencies[]=&qualities[]=&storages[]=");
        $response->assertStatus(200);
    }
    ...
    /**
     * @dataProvider providerWrongQualities
     * @param mixed[] $data
     */
    public function testWrongQualities(array $data): void
    {
        $response = $this->actingAs($this->getAdmim())
            ->withSession(['banned' => false])
            ->get(route("admin.index") . $this->buildQuery($data));
     ...
     * @return mixed[]
     */
    public function providerWrongQualities(): array
    {
        return [
            [
                ["qualities" => 1],
    ...
    /**
     * @dataProvider providerWrongStorages
     * @param mixed[] $data
     */
    public function testWrongStorages(array $data): void
    {
        $response = $this->actingAs($this->getAdmim())
            ->withSession(['banned' => false])
            ->get(route("admin.index") . $this->buildQuery($data));
     ...
     * @return mixed[]
     */
    public function providerWrongStorages(): array
    {
        return [
            [
                ["storages" => 1],
